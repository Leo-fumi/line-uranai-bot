...
def get_fortune_response(user_info, topic):
    ...
    prompt = f"""
    以下のユーザー情報を基に、西洋占星術、東洋占星術、数秘術、姓名判断（熊崎式）を統合的に活用して、非常に詳細かつ充実した占い結果を出力してください。

    占いの結果は以下の係数に基づいて統合してください。

    最終的な占いの結果＝30*A+60*B+10*C+13*D
    ...
    できるだけ豊かな表現と目の前で鑑定しているかのような口調、具体的なアドバイス、未来に向けた励ましのメッセージを含めた、複数の段落に渡る長文の占い結果を生成してください。
    また、各占術の結果が自然に融合し、全体として読み応えのある内容になるようにしてください。ただし、どの占星術による結果かということは書かないでください。
